...
import { 
  FiTool, FiClipboard, FiCheckCircle, FiClock, FiX, FiAlertCircle, FiPlay
} from 'react-icons/fi';
...
const MyJobs = () => {
  ...
  const fetchMyRepairs = async () => {
    try {
      ...
      if (filter === 'active') {
        filtered = response.data.filter(r => 
          r.status === 'pending' || r.status === 'in_progress' || r.status === 'details_approved'
        );
  ...
  const removeSparePartRow = (index) => {
    ...
    setDetailsData({ ...detailsData, spareParts: newParts.length > 0 ? newParts : [{ name: '', quantity: 1, cost: 0 }] });
  };
  ...
  const getStatusBadge = (status) => {
    const badges = {
      pending: { color: 'bg-yellow-100 text-yellow-800', label: 'Assigned to You' },
      in_progress: { color: 'bg-blue-100 text-blue-800', label: 'In Progress' },
      awaiting_details_approval: { color: 'bg-orange-100 text-orange-800', label: 'Awaiting Approval' },
      details_approved: { color: 'bg-purple-100 text-purple-800', label: 'Ready to Complete' },
    ...
    <div className="animate-fade-in">
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-800 mt-1">
              <strong>Step 1:</strong> Click ▶️ "Start Work" when you begin<br />
              <strong>Step 2:</strong> Work on the motorcycle<br />
              <strong>Step 3:</strong> Click 📝 "Register Details" to record parts, labor, and costs<br />
              <strong>Step 4:</strong> Wait for approval (Sales → Admin)<br />
      ...
      >
        ...
        <form onSubmit={handleSubmitDetails}>
                ...
                <div key={index} className="grid grid-cols-12 gap-2">
                  ...
                  <div className="col-span-2">
                    <input
                      ...
                      placeholder="Qty"
                      value={part.quantity}
                      onChange={(e) => updateSparePart(index, 'quantity', parseFloat(e.target.value) || 1)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                      min="1"
                    />
                  </div>
                  <div className="col-span-4">
                    <input
                      ...
                      value={part.cost}
                      onChange={(e) => updateSparePart(index, 'cost', parseFloat(e.target.value) || 0)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                      min="0"
                    />
                  </div>
          ...
          {/* Work Description */}
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">What Work Did You Do? *</label>
            <textarea
          ...
          {/* Issues Found */}
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">Issues Found (If Any)</label>
            <textarea
          ...
          {/* Recommendations */}
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">Recommendations for Customer</label>
            <textarea
          ...
          <div className="bg-orange-50 border border-orange-200 p-4 rounded-lg mb-4">
            <p className="text-sm text-orange-800">
              <strong>⚠️ Important:</strong> After submitting, your costs will be sent for approval (Sales → Admin). 
              You can mark the repair as complete only after approval.
            </p>
          </div>
