...
const Reports = () => {
  ...
  const handleDownloadReport = async (reportType, format = "excel") => {
    ...
    try {
      ...
      if (format === "excel") {
        ...
        link.setAttribute(
          "download",
          `${reportType}-report-${Date.now()}.xlsx`
        );
  ...
  const allReportTypes = [
    {
      title: "Sales Report",
      description: "Comprehensive sales report with revenue analysis",
      type: "sales",
      icon: FiFileText,
      color: "bg-green-500",
      roles: ["all"], // All roles can see
    },
    {
    ...
    },
    {
      ...
      icon: FiFileText,
      color: "bg-indigo-500",
      roles: ["admin"], // Only admin can see profit reports
    },
  ];

  // Filter reports based on user role - Secretary should NOT see profit reports
  const reportTypes = allReportTypes.filter((report) => {
    if (report.roles.includes("all")) return true;
    if (user?.role === "secretary" && report.type === "profit") return false; // Secretary cannot see profit
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reportTypes.map((report) => {
            ...
            return (
              <div
                key={report.type}
                className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
              >
                <div className="flex items-center justify-between mb-4">
                  <div
                    className={`${report.color} p-4 rounded-2xl text-white shadow-lg`}
                  >
                    <Icon size={24} />
                  </div>
                </div>
